...
tags: ["engineering"]
meta:
  title: "Многомасштабное моделирование механизмов повреждения пласта и отклика на матричную кислотную обработку - RockStim"
...
Исторические примеры показывают, насколько тяжелым может быть повреждение. В Prudhoe Bay, Alaska (1980), горизонтальная добывающая скважина получила катастрофическое повреждение после ремонта с несовместимой жидкостью заканчивания: добыча снизилась с 500 до менее 5 баррелей в сутки за две недели. На формации Brent в Северном море (1995) скважина полностью остановилась после закачки пластовой воды с высоким содержанием бария, вызвавшей массивное выпадение сульфата бария и снижение проницаемости с 200 мД до менее 0.1 мД. На бразильских подсолевых месторождениях (2015) разведочная скважина полностью прекратила добычу после взаимодействия цемента с пластовыми флюидами, причем повреждение распространилось на радиус 10 м.
...
где `V_clay` — объемная доля глин, `k_clay` — константа скорости набухания, `V_clay,max` — максимальный объем глин, `C_salt` — концентрация соли, n — порядок реакции.
...
где `C_s` — концентрация растворенного вещества, `D` — коэффициент дисперсии, `k_precip` — константа скорости осаждения, `IAP` — произведение ионной активности, `K_sp` — произведение растворимости.
...
где `k_as` — скорость поверхностной реакции, `C_A,eq` — равновесная концентрация, m — порядок реакции.

### 3.4. Риски вторичного повреждения при кислотной обработке

Хотя матричная кислотная обработка направлена на восстановление проницаемости, некорректный проект обработки может вызвать вторичное повреждение. Эти риски кольматации необходимо включать в модель.
...
где `SI` — индекс насыщения, `IAP` — произведение ионной активности, `K_sp,i` — произведение растворимости потенциального осадка i.
...
где `C_ppt` — концентрация осадка, `k_ppt` — константа скорости осаждения, `SI_crit` — критический индекс насыщения для зародышеобразования, n — порядок реакции, `f(T, pH)` — функция зависимости от температуры и pH.
...
4. Асфальтены и кислотный шлам: выпадают при контакте кислоты с нефтью без достаточного количества противошламовых добавок
...
с `k_mob` — коэффициентом мобилизации.
...
где `P_cr` — критическое давление разрушения, `σ_t` — прочность пласта на растяжение, `r_w` и `r_wh` — радиусы ствола и червоточины, n — эмпирический показатель, `f(π_wh)` — функция пористости червоточины.
...
где `k_as` — скорость поверхностной реакции, `k_mt` — коэффициент массопереноса. Оптимальное червоточинообразование наблюдается в узком диапазоне `Da` (обычно 0.1-10), тогда как поверхностное растворение доминирует при высоких `Da` (>100).
...
Моделирование показывает, что набухание глин преимущественно снижает проницаемость ПЗП, обычно на радиусе 0.5-2.0 м от стенки скважины. Обработки HF-HCl эффективно восстанавливают проницаемость: 70-85% восстановления при 3-5% HF в сочетании с 12-15% HCl и оптимальном времени контакта 2-4 часа.

### 5.2. Миграция частиц

Миграция мелких частиц создает более глубокие зоны повреждения, чем набухание глин, обычно 2-5 м по радиусу. Эффективные обработки требуют более низких концентраций кислоты, чтобы избежать быстрого фронта реакции, обходящего поврежденные зоны; оптимальны длительные контакты 4-6 часов и последовательные протоколы HCl/HF.

### 5.3. Устранение химического осадкообразования

Осаждение карбоната кальция имеет сложные пространственные профили, зависящие от зон смешения и локальной химии. Эффективные обработки используют хелатирующие агенты, такие как EDTA или GLDA, в сочетании с предварительной промывкой HCl; температурный диапазон 50-70°C оптимален для максимизации растворения и снижения нежелательных побочных реакций.
...
Анализ чувствительности показывает, что избыточные концентрации HF в терригенных обработках (>5%) вместе с недостаточной предварительной промывкой резко повышают риск выпадения силикагеля и фторида кальция. Модель прогнозирует, что каждый 1% роста HF выше оптимума может снизить проницаемость в критических зонах на 15-30%.

Промысловая валидация подтверждает прогнозы: в примере Северного моря избыточная HF-концентрация (7%) вызвала сильное вторичное повреждение, снизив продуктивность после обработки ниже исходного уровня. Карта проницаемости показала кольцевую зону вторичного повреждения на расстоянии примерно 0.3-0.7 м от ствола, совпадающую с расчетным фронтом осаждения.

### 5.4.2. Миграция частиц, вызванная кислотой

Параметрические исследования показывают, что высокие темпы закачки, создающие резкие pH-градиенты (>3 единиц pH/см), увеличивают мобилизацию частиц на 300-500% по сравнению с оптимизированными протоколами. Критический порог возникает при темпах выше 0.25 барр./мин/фт толщины пласта в песчаниках с содержанием глин более 8%.

Показательный пример в Мексиканском заливе: высокотемповая кислотная закачка (0.5 барр./мин/фт) вызвала катастрофическую миграцию частиц и полностью нивелировала эффект обработки. Повторная обработка с модельно-оптимизированным темпом (0.15 барр./мин/фт) и ступенчатым повышением концентрации успешно восстановила добычу без запуска вторичной миграции.
...
Интегрированная модель показывает масштабную зависимость повреждения и кислотного отклика. На поровом масштабе доминируют быстрые реакции, а на масштабе Дарси общий процесс контролируется ограничениями переноса. ПЗП действует как критический фильтр, где концентрируются все эффекты и формируется максимальное сопротивление потоку.

### 5.6. Случаи сильного повреждения и восстановление

На терригенных пластах Суматры отмечалось катастрофическое повреждение при ГРП: добыча снизилась с 800 до 0 баррелей в сутки за 72 часа из-за массивного выпадения алюмосиликатов и образования силикагеля. Восстановительная обработка включала модифицированную HF-HCl кислотную систему, фторсульфоновую кислоту и специальные ПАВ, что восстановило 65% продуктивности.
...
Скважина в терригенном пласте с высоким содержанием глин показала практическую применимость модели. Исходная продуктивность 0.5 барр./сут/psi из-за сильного глинистого повреждения выросла до 3.2 барр./сут/psi при использовании оптимизированной системы HF-HCl, что соответствует шестикратному улучшению.
...
Комплексный минералогический анализ задает критические пороги выбора кислоты и концентрации. XRD и SEM-EDX дают данные о типах глин, акцессорных минералах и предшественниках потенциальных осадков. Испытания в тонкой трубке оценивают совместимость кислоты и нефти, а капиллярное время всасывания (CST) — чувствительность к мобилизации мелких частиц.

#### 6.3.2. Многостадийное проектирование обработки
...
Промысловая валидация сформировала рабочий процесс: диагностика типа повреждения по анализу неустановившегося давления, выбор кислотной системы по механизму повреждения, оптимизация темпа закачки по модели коллектора и корректировки по данным мониторинга в реальном времени.

### 6.5. Предотвращение тяжелого повреждения

Протоколы предотвращения, основанные на экстремальных примерах, включают детальную характеристику пласта, тестирование совместимости флюидов, строгую фильтрацию закачиваемой воды, непрерывный мониторинг и планы быстрого реагирования со специализированными кислотными системами и подготовленными бригадами.

## 7. Ограничения и допущения модели

### 7.1. Фундаментальные ограничения

Модель учитывает вариации поля проницаемости, но предполагает пространственную непрерывность твердой фазы и недостаточно описывает сложные геологические структуры, такие как разломы, естественные трещины или слои с экстремальным контрастом свойств. Неопределенности входных данных, особенно кинетических параметров, которые трудно определить непосредственно в пласте, дают около 30% ошибки прогноза при ограниченных данных характеристики.

Физические ограничения включают неполный учет температурных эффектов при фазовых изменениях и сложной кинетике, отсутствие полной геомеханической связи напряжений и уплотнения, а также упрощение электрохимических взаимодействий глинистых минералов. Модель предполагает локальное термодинамическое равновесие, ламинарный режим при числе Рейнольдса ниже 10, изотропную проницаемость на масштабе Дарси и несжимаемые флюиды при изменениях давления ниже 10% от начального.

### 7.2. Стратегии снижения неопределенности

Для сильно неоднородных пластов модель использует статистическое укрупнение параметров по функциям потока, методы гомогенизации для зон с высоким контрастом и многоконтинуумные подходы для трещиноватых зон. Проблемы точности данных решаются через анализ чувствительности методом Монте-Карло, байесовскую калибровку по историческим данным добычи и применение коэффициентов запаса в протоколах обработки.

### 7.3. Валидация в экстремальных условиях

Полевые тесты при сильном повреждении показали точность 70-85%. Диапазоны применимости включают проницаемость 0.1-5000 мД, глубины 500-4000 м, температуры 20-150°C и pH флюидов от 0 до 7.

## 8. Выводы

Разработанная многомасштабная модель дает комплексный подход к пониманию и оптимизации матричных кислотных обработок для разных типов повреждения пласта с учетом фундаментальных ограничений. Включение механизмов вторичного повреждения существенно улучшает проектирование обработки, позволяя выявлять и снижать риски осадкообразования и миграции частиц. Ключевые выводы: преимущество профилактики через корректную характеристику пласта, необходимость индивидуальных протоколов для экстремальных случаев и критическая роль многомасштабной интеграции при анализе сложного повреждения.
...
3. Разработаны многостадийные проекты обработки, минимизирующие pH-шоки и скачки ионной силы
...
Дальнейшее развитие должно включать геомеханическую связь для учета уплотнения и инициирования трещин, вызванных повреждением, внедрение искусственного интеллекта для автоматического распознавания закономерностей повреждения и прогноза их эволюции в реальном времени, а также расширение модели на трещиноватые коллекторы с учетом связи матрица-трещина. Перспективны распределенное оптоволоконное зондирование, наночастичные трассеры и высокопроизводительные вычисления на графических процессорах.

Дополнительно следует исследовать инновационные системы добавок для предотвращения вторичного повреждения: оптимизацию хелатирующих агентов, современные стабилизаторы глин и интеллектуальные инкапсулированные системы, высвобождающие стабилизаторы в ответ на локальные химические условия. Интеграция с машинным обучением позволит постоянно улучшать модель по промысловым результатам и создавать обратную связь для все более эффективных проектов обработки.
